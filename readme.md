...
|Email         | VARCHAR(64)    | Почта пользователя      | Обязательное поле, уникальный  |
...
| Имя поля | Тип    | Описание  | Ограничения   |
|----------|---------|---------|---------------|
|CustomerID|INT        | Первичный ключ      | Обязательное поле, уникальный, авто-инкрементация              |
|UserID    |INT        | Внешний ключ к "UserBase" , для получения данных основы пользователя (пароль, почта) |Обязательное поле|
...
|CustomerID   |INT          | Внешний ключ к "Customer" , для хранения ID заказавшего пользователя |Обязательное поле |
|Total price  |INT         | Общая стоимость товаров в заказе |Обязательное поле |
...
| Имя поля | Тип    | Описание  | Ограничения   |
|----------|---------|---------|---------------|
|AdminID   |INT  | Первичный ключ    |Обязательное поле, уникальный, авто-инкрементация |
|UserID    |INT  | Внешний ключ к "UserBase" , для получения данных основы пользователя (пароль, почта) |Обязательное поле |
